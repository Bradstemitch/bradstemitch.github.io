...
function CreatureStatBlockDnD5e(props: any) {
    let creature = require(`../../../data/creatures/DnD5e/fvtt-zzzErrorDefault.json`);
    let error = false
    try {
        creature = require(`../../../data/creatures/DnD5e/fvtt-${props.creature.fileName}.json`);
    } catch (e) {
        creature = require(`../../../data/creatures/DnD5e/fvtt-zzzErrorDefault.json`);
        error = true
    }
                    ...
                    {creature.items.filter((item: any) => item.name === 'Multiattack').map((item: any) => (
                        <div key={item.name + '-creatureStatBlockDnD5e-' + creature.name + '-div'}>
                            <span key={item.name + '-creatureStatBlockDnD5e-' + creature.name + '-span'}>
                                ...
                                <span dangerouslySetInnerHTML={{ __html: item.system.description.value }} key={item.name + '-creatureStatBlockDnD5e-' + creature.name + '-span2'} />
                            </span>
                        </div>
                    ))}
